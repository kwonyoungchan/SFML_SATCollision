...
BoxCollider::BoxCollider():Collider(4)
{
}
...
void BoxCollider::Create(sf::Vector2f size)
{
	...
	v[0] = pos + sf::Vector2f(.0f, .0f);
	v[1] = pos + sf::Vector2f(size.x, .0f);
	v[2] = pos + sf::Vector2f(size.x, size.y);
	v[3] = pos + sf::Vector2f(.0f, size.y);
}
...
void BoxCollider::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	target.draw(v, 6, sf::PrimitiveType::LineStrip);
}

CircleCollider::CircleCollider():Collider(1)
{
}
...
void CircleCollider::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	target.draw(v, points + 1, sf::PrimitiveType::LineStrip);
	sf::Vertex vertex({ m_position, ColliderColor });
	target.draw(&vertex, 1, sf::PrimitiveType::Points);
}


CustomCollider::CustomCollider(): Collider(0)
{
}
...
sf::Vector2f* CustomCollider::GetVerticles()
{
	return m_vertices;
}

sf::Vector2f& CustomCollider::GetVerticles(uint32_t index)
{
...
void CustomCollider::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	target.draw(v, count, sf::PrimitiveType::LineStrip);
