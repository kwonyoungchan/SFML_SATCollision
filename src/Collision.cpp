...
bool Collision::SATCollision(const Collider& body, const Collider& other, sf::Vector2f& MTV)
{
	...
	// 도형이 원인 경우 처리하는 분기 
	if (all == 2) return SATCollision((const CircleCollider&)body, (const CircleCollider&)other, MTV);
	else if (bodyCount == 1) return SATCollision((const CircleCollider&)body, other, MTV);
	else if (otherCount == 1) return SATCollision(body, (const CircleCollider&)other, MTV);
	...
	for (uint32_t i = 0; i < all; i++)
	{
		...
		sf::Vector2f bodyProtection = ProjectOnto(bodyVertices, bodyCount, a);
		sf::Vector2f otherProtection = ProjectOnto(otherVertices, otherCount, a);
...
bool Collision::SATCollision(const CircleCollider& body, const Collider& other, sf::Vector2f& MTV)
{
	...
	if (DotProduct(center - GetCenter(other), MTV) < .0f)
		MTV *= -1.0f;
